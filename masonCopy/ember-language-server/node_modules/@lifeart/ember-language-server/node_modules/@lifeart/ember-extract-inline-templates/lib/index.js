...
exports.getTemplateNodes = exports.searchAndExtractHbs = exports.defaultHbsTagSources = void 0;
const traverse_1 = require("@babel/traverse");
exports.defaultHbsTagSources = {
 ...
 */
function searchAndExtractHbs(source, options) {
    const templateNodes = getTemplateNodes(source, options);
    // no inline template(s) found !
    if (templateNodes.length === 0) {
        return '';
    }
    const hbsSource = _toHbsSource(templateNodes);
    return hbsSource;
}
exports.searchAndExtractHbs = searchAndExtractHbs;
/**
 ...
 * plugin for **typescript syntax**.
 */
function getTemplateNodes(source, options) {
    const hbsTagSources = (options.hbsTagSources)
        ? Object.assign(Object.assign({}, exports.defaultHbsTagSources), options.hbsTagSources) : exports.defaultHbsTagSources;
    ...
    const AST = _getAST(source, options);
    const hbsTags = _getHbsTags(AST, hbsTagSources);
    // the script doesn't have import declaration(s) or none of the import declaration is a valid hbs tag !
    if (hbsTags === false) {
        return [];
    }
    const templateNodes = _getTemplateNodes(AST, hbsTags, sortByStartKey);
 ...
 * plugin for **typescript syntax**.
 */
function _getAST(source, options) {
 ...
 */
function _getTemplateNodes(AST, hbsTags, sortByStartKey = false) {
    let templateNodes = [];
        ...
        TaggedTemplateExpression({ node }) {
            ...
            if (tag && tag.type === "Identifier" && hbsTags.includes(tag.name)) {
                ...
                if (innerNode.loc !== null) {
                    templateNodes = [...templateNodes, Object.assign({ template: innerNode.value.raw, startLine: innerNode.loc.start.line, startColumn: innerNode.loc.start.column, endLine: innerNode.loc.end.line, endColumn: innerNode.loc.end.column }, innerNode)];
                }
            }
        },
        CallExpression({ node }) {
            ...
            node.arguments.forEach((argument) => {
                if (callee.type === "Identifier" &&
                    ...
                    switch (argument.type) {
                        case "StringLiteral":
                            if (argument.loc) {
                                templateNodes = [...templateNodes, Object.assign({ template: argument.value, startLine: argument.loc.start.line, 
                                        ...
                                        startColumn: argument.loc.start.column + 1, endLine: argument.loc.end.line, endColumn: argument.loc.end.column }, argument)];
                            }
                            break;
                        case "TemplateLiteral":
                            if (argument.quasis[0].loc) {
                                templateNodes = [...templateNodes, Object.assign({ template: argument.quasis[0].value.raw, startLine: argument.quasis[0].loc.start.line, startColumn: argument.quasis[0].loc.start.column, endLine: argument.quasis[0].loc.end.line, endColumn: argument.quasis[0].loc.end.column }, argument.quasis[0])];
                            }
                            break;
                    }
                }
 ...
 */
function _getHbsTags(AST, hbsTagSources) {
    ...
    const hbsTagSourcesKeys = Object.keys(hbsTagSources);
    const hbsTags = importDeclarationNodes.reduce((acc, node) => {
        ...
        if (hbsTagSourcesKeys.includes(nodeSourceValue)) {
            ...
            if (wantedSourceSpecifier === 'default') {
                const importDefaultSpecifiers = node.specifiers.filter(e => e.type === 'ImportDefaultSpecifier');
                const [defaultSpecifier] = importDefaultSpecifiers;
                if (defaultSpecifier) {
                    const defaultTagName = defaultSpecifier.local.name;
                    acc = [...acc, defaultTagName];
                    return acc;
                }
            }
            const importSpecifiers = node.specifiers.filter(e => e.type === 'ImportSpecifier');
            const specifiers = importSpecifiers.filter((specifier) => {
            ...
            if (specifiers.length) {
                specifiers.forEach((specifier) => {
                    const localTagName = specifier.local.name;
 ...
 */
function _sortTemplateNodesByStartKey(templateNodes) {
 ...
 */
function _toHbsSource(templateNodes) {
