...
exports.DISABLE_RULE_TEXT_EDITS = {
    [constants_1.DisableRuleCommandIds.applyDisableRuleInline]: (document, range, rule) => {
        ...
        const line = document.getText(node_1.Range.create(node_1.Position.create(lineNr, 0), node_1.Position.create(lineNr + 1, 0)));
        const character = line.length - (line.endsWith("\r\n") ? 2 : line.endsWith("\n") ? 1 : 0);
        const newText = ` /* stylelint-disable-line ${rule} */`;
        return [node_1.TextEdit.insert(node_1.Position.create(lineNr, character), newText)];
    },
    [constants_1.DisableRuleCommandIds.applyDisableRuleToLine]: (document, range, rule) => {
        ...
        return [node_1.TextEdit.insert(node_1.Position.create(lineNr, 0), newText)];
    },
    [constants_1.DisableRuleCommandIds.applyDisableRuleToFile]: (_document, _range, rule) => {
        const newText = `/* stylelint-disable ${rule} */\n`;
        return [node_1.TextEdit.insert(node_1.Position.create(0, 0), newText)];
    },
    [constants_1.DisableRuleCommandIds.applyDisableRuleToRange]: (document, range, rule) => {
        const startLineNr = range.start.line;
        const startLine = document.getText(node_1.Range.create(node_1.Position.create(startLineNr, 0), node_1.Position.create(startLineNr + 1, 0)));
        const startIndent = getIndent(startLine);
        const startNewText = `${startIndent}/* stylelint-disable ${rule} */\n`;
        const endLineNr = range.end.line;
        let endLine = document.getText(node_1.Range.create(node_1.Position.create(endLineNr, 0), node_1.Position.create(endLineNr + 1, 0)));
        if (endLine === "" || endLine === "\r\n" || endLine === "\n") {
            endLine = document.getText(node_1.Range.create(node_1.Position.create(endLineNr - 1, 0), node_1.Position.create(endLineNr, 0)));
        }
        const endIndent = getIndent(endLine);
        const endNewText = `${endIndent}/* stylelint-enable ${rule} */\n`;
        return [
            node_1.TextEdit.insert(node_1.Position.create(startLineNr, 0), startNewText),
            node_1.TextEdit.insert(node_1.Position.create(endLineNr + 1, 0), endNewText),
        ];
    },
};
...
function buildCodeActionHandler(documents, settings) {
    return (params) => {
        ...
        if (params.context.only) {
            returnSource = params.context.only.includes(node_1.CodeActionKind.Source);
            returnQuickFixes = params.context.only.includes(node_1.CodeActionKind.QuickFix);
        }
        if (returnSource && applyToWholeFile) {
            const supportsCodeActionLiterals = settings.supportedCodeActionLiterals.includes(node_1.CodeActionKind.Source);
            const title = constants_1.CommandTitles[constants_1.CommandIds.applyAutoFixes];
            const command = node_1.Command.create(title, constants_1.CommandIds.applyAutoFixes, node_1.VersionedTextDocumentIdentifier.create(document.uri, document.version));
        ...
        if (returnQuickFixes) {
            ...
            const disableRuleCommands = applyToLine
                ...
                : applyToWholeFile
                    ? exports.DISABLE_RULES_FOR_WHOLE_FILE
                    : exports.DISABLE_RULES_FOR_RANGE;
            const supportsCodeActionLiterals = settings.supportedCodeActionLiterals.includes(node_1.CodeActionKind.QuickFix);
            rules.forEach((diagnostics, rule) => {
                disableRuleCommands.forEach((disableKey) => {
                    ...
                    if (supportsCodeActionLiterals) {
                        ...
                        results.push(node_1.Command.create(title, disableKey, node_1.VersionedTextDocumentIdentifier.create(document.uri, document.version), range, rule));
                    }
                });
            });
        }
...
function buildExecuteCommandHandler(connection, messageQueue, documents, settings) {
    return async (params) => {
        ...
        if (isApplyAutoFixesCommand(params)) {
            ...
            edits = await (0, validate_1.autoFix)(document, config);
        }
        ...
        if (document && edits.length) {
            if (!settings.supportsApplyEdit) {
                connection.console.warn(`${edits.length} edits to apply, but client does not support applying them`);
                return;
            }
            ...
            if (result.applied) {
                ...
                if (document) {
                    (0, validate_1.validateDocument)(messageQueue, document);
                }
            }
            else {
...
function registerCommandHandlers(connection, messageQueue, documents, settings) {
    ...
    messageQueue.onRequest(node_1.ExecuteCommandRequest.type, buildExecuteCommandHandler(connection, messageQueue, documents, settings), ({ arguments: args }) => args && args[0] && args[0].version);
}
