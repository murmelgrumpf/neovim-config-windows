...
exports.validateNotification = new node_1.NotificationType("stylelint/validate");
...
async function validate(connection, document, settings) {
    ...
    connection.sendDiagnostics({
        ...
        diagnostics: result.warnings.map((warning) => {
            const position = node_1.Position.create(warning.line - 1, warning.column - 1);
            return {
                code: warning.rule,
...
async function autoFix(document, settings) {
    ...
    let cur = 0;
    (0, fast_diff_1.default)(originalText, output).forEach(([action, str]) => {
    ...
    return changes.map(({ start, end, newText }) => node_1.TextEdit.replace(node_1.Range.create(document.positionAt(start), document.positionAt(end)), newText));
}
